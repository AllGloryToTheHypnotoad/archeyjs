...
function getIP(){
	...
	// 		// if (net['usb0']) return ip('usb0');
	// 		ret = [];
	// 		if (net['eth0']) ret.push(ip('eth0'));
	// 		if (net['wlan0']) ret.push(ip('wlan0'));
	// 		if (net['wlan1']) ret.push(ip('wlan1'));
	// 		if (net['usb0']) ret.push(ip('usb0'));
	// 		return ret;
	// 		break;
	// 	case 'darwin':
	// 		ret = [];
	// 		if (net['en0']) ret.push(ip('en0'));
	// 		if (net['en1']) ret.push(ip('en1'));
	// 		return ret;
	// 		break;
	// 	default:
	...
	// ok - can't find anything useful, get the loopback interface
		...
		if (net['lo0']) return ip('lo0'); // apple
		if (net['lo']) return ip('lo');   // linux
	}
}
